...
function alphabetize_dropdown(obj) {
    ...
    let userdefined =$(obj).find('option:--- User Uploaded ---')
...
// Needs better fix for unique Identifier
let payloadParser =  function(){
    var element = document.getElementById("ability-payload");
    for(var i=0; i<element.length; i++ ){
        if(element.options[i].text.includes('_P_')){
            var usertext = element.options[i].text.split('_P_')[0];
            element.options[i].text = usertext;
        }
    }
}
// Currenlty calling here ... need to call maybe from id="ability-ability-filter" or ability-payload template
payloadParser();
