...
Raises DataValidationError (caught by v2 middleware as HTTP 400).
"""
import pytest
...
class TestSanitizeId:
    ...
    def test_non_string_id_rejected(self):
        """Non-string IDs (e.g. int, None) should raise DataValidationError, not TypeError."""
        with pytest.raises(DataValidationError):
            BaseApiManager._sanitize_id(12345)
        with pytest.raises(DataValidationError):
            BaseApiManager._sanitize_id(None)
