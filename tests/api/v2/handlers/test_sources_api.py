...
@pytest.fixture
def updated_source_payload(test_source):
    source_data = test_source.schema.dump(test_source)
    new_fact = Fact(trait='new_test_fact', value=2)
    source_data.update(dict(name='an updated test source', facts=[new_fact]))
    return source_data
...
def replaced_source_payload(test_source):
    source_data = test_source.schema.dump(test_source)
    source_data.update(dict(name='replaced test source', facts=[], relations=[]))
    return source_data


@pytest.fixture
def test_source(loop, api_v2_client, executor):
    fact = Fact(trait='test_fact', value=1)
    rule = Rule(RuleAction.ALLOW, trait='test_rule')
...
class TestSourcesApi:
    ...
    async def test_create_source(self, api_v2_client, api_cookies, mocker, async_return, new_source_payload):
