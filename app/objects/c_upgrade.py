...
#     def __init__(self, upgrade_id, name, module, params, stopping_conditions=None, description=None,
...
#     def store(self, ram):
#         existing = self.retrieve(ram['upgrade'], self.unique)
#         if not existing:
#             ram['upgrade'].append(self)
#             return self.retrieve(ram['upgrade'], self.unique)
#         else:
...
#             if await self.walk_file_path(os.path.join('plugins', plugin, 'data', ''), '%s.yml' % self.upgrade_id):
#                 return plugin
#         return None
