...
class AgentService(BaseService):
    ...
    async def perform_action(self, link: typing.Dict) -> int:
        ...
        while operation['state'] != operation_svc.op_states['RUNNING']:
            if operation['state'] == operation_svc.op_states['RUN_ONE_LINK']:
                ...
                await self.data_svc.dao.update('core_operation', 'id', op_id, dict(state=operation_svc.op_states['PAUSED']))
                return link_id
            else:
                await asyncio.sleep(30)
                operation = (await self.data_svc.dao.get('core_operation', dict(id=op_id)))[0]
        link.pop('adversary_map_id')
        return await self._create_link(link)
    ...
    async def _create_link(self, link):
        ...
        for fact in facts:
            await self.data_svc.create('core_fact_usage', dict(link_id=link_id, fact_id=fact['id']))
        return link_id
