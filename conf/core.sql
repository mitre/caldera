...
CREATE TABLE if not exists core_agent (id integer primary key AUTOINCREMENT, paw text, username text, hostname text, last_seen date, architecture text, platform text, server text, host_group text, location text, pid integer, ppid integer, trusted integer, last_trusted_seen date, sleep integer);
