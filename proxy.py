...
def render_proxy_config(cfg):
    ...
    rendered = 'haproxy-%s-rendered.conf' % datetime.now().strftime('%Y%H%M%S')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-CI', '--caldera_ip', required=True, default='localhost', help='IP address of the caldera'
                                                                                        ' server')
    parser.add_argument('-CP', '--caldera-port', required=True, default=8888, help='Port on which the caldera server is'
                                                                                   'serving http')
